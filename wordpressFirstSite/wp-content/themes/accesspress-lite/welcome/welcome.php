		...
		class Accesspresslite_Welcome {
			...
			public function accesspresslite_activation_admin_notice() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					<div class="notice notice-success is-dismissible">
						<p><?php /* translators: %1$s : theme name, %2$s : theme version */ printf( wp_kses(__( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'accesspress-lite' ), array( 'a' => array( 'href' => array() ) ) ), esc_html($this->theme_name), esc_url(admin_url( 'themes.php?page=accesspresslite-welcome' ))  ); ?></p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=accesspresslite-welcome' )); ?>"><?php esc_html_e( 'Lets Get Started', 'accesspress-lite' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function accesspresslite_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php /* translators: %1$s : theme name, %2$s : theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'accesspress-lite' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
					<div class="about-text"><?php /* translators: %s : theme name */ printf( esc_html__( 'The %s is a responsive multipurpose WordPress business theme with clean, highly professional design and lots of useful features like a premium theme - and strong theme options panel to manage theme all! It is probably the most feature-rich free theme with lots of useful options events layout, portfolio layout, testimonial layout, blog layout, gallery layout, featured posts on home page, quick contact, social media integration, full width slider, team member layout, sidebar layout, multiple home page layout, call to action and many other page layouts.', 'accesspress-lite' ), esc_html($this->theme_name) ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=accesspresslite-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function accesspresslite_plugin_activation_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'accesspress-lite' ) );
				...
				if (! wp_verify_nonce( $nonce, 'accesspresslite_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'accesspress-lite' ) );
